...
fun Any.complete(previous: PreviousHolder, builder: (String) -> String) {
    ...
    if (diff > 1000000) Globals.logger.debug  { builder("%.3f ms".format(diff.toDouble()/1000000)) }
    else if (diff > 1000) Globals.logger.debug  { builder("%.3f us".format(diff.toDouble()/1000)) }
    else Globals.logger.debug { builder("$diff ns") }
