 ...
 * src: <a href="https://github.com/frengor/LightInjector/blob/main/src/main/java/com/fren_gor/lightInjector/LightInjector.java">LightInjector</a>
 * <p>
...
public abstract class TinyProtocol {
    ...
    @Nullable
    private static final Field NMS_PENDING_NETWORK_MANAGERS = getPendingNetworkManagersFieldOrNull();
     ...
     */
    public TinyProtocol(@NotNull Plugin plugin) {
        ...
        synchronized (networkManagers) { // Lock out Minecraft
            for (Object manager : networkManagers) {
                try {
                    ...
                    try (EventLoop eventLoop = channel.eventLoop()) {
                        eventLoop.submit(() -> channel.pipeline().remove(identifier));
                    } catch (Exception exception) {
    ...
    private static Field getField() {
        try {
            Field f = TinyProtocol.SERVER_CONNECTION_CLASS.getDeclaredField("pending");
            f.setAccessible(true);
            return f;
        } catch (ReflectiveOperationException exception) {
            throw new RuntimeException("[LightInjector] Can not find field! (" + TinyProtocol.SERVER_CONNECTION_CLASS.getName() + '.' + "pending" + ')', exception);
        }
    }
    ...
    private static Field getPendingNetworkManagersFieldOrNull() {
        ...
        try {
            return getField(TinyProtocol.SERVER_CONNECTION_CLASS, Queue.class, 1);
        } catch (Exception ignored) {
        }
        try {
            return getField(TinyProtocol.SERVER_CONNECTION_CLASS, List.class, 3);
        } catch (Exception ignored) {
        }
        return null;
    }
